...
class Card extends Component {
  ...
  render() {
    ...
    const op = store.getState().cards[id]
      ? store.getState().cards[id].opacity
      : 1;
    const classStr = store.getState().cards[id]
      ? store.getState().cards[id].classStr
      : "role-card";
    return (
      ...
      <div className={"Card " + pos}>
        {role in roleImgMap && role !== "" ? (
          ...
          <img
            className={"role-card placeholder"}
            ref={(elem) => (this.card = elem)}
            src={roleImgMap[role]}
            alt={role}
            style={{ opacity: op }}
